...
func getInterface(interfaces []*current.Interface, containerIfName string, netns ns.NetNS) (*current.Interface, error) {
//	fmt.Printf("testing %s", containerIfName)

        if len(interfaces) == 0 {
                return nil, fmt.Errorf("no interfaces provided")
        }
//        for _, iface := range interfaces {
//		fmt.Printf("against %s", iface.Name)
//	}

	for _, iface := range interfaces {
                if iface.Name == containerIfName { //link.Attrs().Name {
                        return iface, nil
                }
        }

	return nil, fmt.Errorf("no interface found in ns")
}
...
func cmdAdd(args *skel.CmdArgs) error {
	...
	hostInterface, err := getInterface(result.Interfaces, args.IfName, netns)
	if err != nil {
		return err
	}
